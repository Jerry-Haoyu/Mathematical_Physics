...
\end{mystthmdefinition}
\begin{mystexercise}{Completeness of $\mathcal M_{\mu^*}$}{completeness-of---mathcal-m_--mu------hw_mineyev-}\label{Completeness of $\mathcal M_{\mu^*}$[^hw_mineyev]}\label{completeness of $\mathcal m_{\mu^*}$[^hw_mineyev]}A measure $\mu$ on a $\sigma$-algebra $\mathcal{M}$ is complete if
\begin{equation}
...
\end{mystexercise}
\begin{mystsolution}{Solution to Exercise~\ref{Completeness of $\mathcal M_{\mu^*}$[^hw_mineyev]}}\begin{mystthmproof}{}{proof-measure-6}Note that $\mu(N)=0\Longrightarrow \mu(N')=0$. $\forall E\subseteq X$, by subadditivity:
\begin{equation}
...
\end{mystthmdefinition}
\begin{mystexercise}{h-intervals generate $\mathcal B_{\mathbb{R}}$}{exercise-klondizgca}\label{exercise-kLONDizgcA}\label{exercise-klondizgca}Check $H$ is an elementary set and so it generates an algebra $\mathcal A$. Furthermore, show that the $\sigma$-algebra generated by $\mathcal A$ is   $\mathcal B_{\mathbb{R}}$

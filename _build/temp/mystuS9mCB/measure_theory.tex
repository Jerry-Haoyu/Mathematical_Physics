...
\title{\Huge \textbf{Measure Theory}}
...
\subsection{Elementary collection and pre-measure}
...
\end{definition}\subsection{Outer Measure}
...
\end{definition}\begin{mystexercise}\label{exercise-Gd3KAZNmAN}\label{exercise-gd3kaznman}\mystExHead{h-intervals generate $\mathcal B_{\mathbb{R}}$}Check $H$ is an elementary set and so it generates an algebra $\mathcal A$. Furthermore, show that the $\sigma$-algebra generated by $\mathcal A$ is   $\mathcal B_{\mathbb{R}}$
...
\subsection{Step 1: Construction of a premeasure}

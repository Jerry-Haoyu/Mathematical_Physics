...
\begin{mystadmonition}{mystAmber}{Newtonian and Lagrangian Mechanics}\begin{itemize}
...
\begin{mystthmexample}{The lagrangian for the vibrating string}{example-eulerlagrangeequation-1}Let $u$ be deviation from the equilibrium position
...
\begin{mystthmtheorem}{Euler-Lagrange Equation}{theorem-eulerlagrangeequation-2}$y$ is a stationary point for
\begin{equation}
...
\end{mystthmtheorem}
\subsubsection{Discretized Euler-Lagrange Equation}

\subsubsection{First Integral}

\begin{mystthmdefinition}{First Integral}{definition-eulerlagrangeequation-3}Suppose
\begin{equation}
y^{(n)}=f(x,y,y^{(1)},...,y^{(n-1)})
\end{equation}
...
\begin{mystadmonition}{mystGreen}{The particular form that allows easy first integral}If we assume $f$ is only dependent on $y,y'$, the above becomes $f(y,y')$, then
\begin{equation}
\begin{aligned}
...
\begin{mystthmtheorem}{Euler-Lagrange Equation}{theorem-eulerlagrangeequation-4}Consider the functional
\begin{equation}
...
\begin{mystadmonition}{mystBlue}{Note}\footnote{From Dr. P.Draper's homework problems for PHYS 508(\textit{Mathematical Physics I}) at UIUC.}\textbf{Fermat's Principle}\\
The path taken by a ray of light between any two point makes stationary the travel time between those points.

\end{mystadmonition}
We also define:

\begin{mystthmdefinition}{Refractive Index}{definition-eulerlagrangeequation-5}A medium is characterized by the speed of light traveling through. Specifically, the refractive index is defined by $n=\frac{c}{\text{speed of light traveling in the medium}}$

\end{mystthmdefinition}
\begin{mystadmonition}{mystAmber}{\textbf{Problem 1} Light Propagtes in Straight Line in Homogenous Media}\begin{enumerate}
\item \textbf{Show that light propagates along a stragiht line in homogenous media.}
\end{enumerate}

\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}Since $n$ is independent of position in homogenous media, make stationary the travel time is equivalent to make stationary the length of the path taken. Now the problem is equivalent to optimizing:
\begin{equation}
...
\end{mystadmonition}
\begin{mystadmonition}{mystAmber}{\textbf{Problem 2} Snell's Law}Consider the propagation of light from a flat slab of glass of refractive index $n_1$ into to
...
\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}We consider the first point at $(x_1, 0)$, the second point at $(x_2,y_2)$ where $x_1<0<x_2$. The time taken is hence:
\begin{equation}
...
\end{mystadmonition}
\begin{mystadmonition}{mystAmber}{\textbf{Problem 3} General Snell's law}A planar light ray propagates in an layered medium with refractive index $n(x)$. Use Fermat's principle to establish a generalized Snell's law:
\begin{equation}
...
\end{mystadmonition}
\paragraph{Drums and Membranes }

\footnote{From Dr. P.Draper's homework problems for PHYS 508(\textit{Mathematical Physics I}) at UIUC.}The shape of a distorted drumskin is described by the
function $h(x, y)$, which gives the height to which the point $(x, y)$ of the flat undistorted drumskin is displaced.

\begin{mystadmonition}{mystAmber}{\textbf{Problem 1} Area of distorted drumskin}Show that the area of the distorted drumskin is equal to
\begin{equation}
A[h] = \int dxdy \sqrt{1 + \partial_x h^2 + \partial_y h^2}
\end{equation}

\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}Consider the area differntial $dA=ds_x ds_y$ where $ds_x=\sqrt{1+\partial_x h^2}dx$ and $ds_y=\sqrt{1+\partial_y h^2}dy$ hence
\begin{equation}
...
\end{mystadmonition}
\begin{mystadmonition}{mystAmber}{Area for small distortion}Show that for small distortions, the area reduces to
\begin{equation}
\label{afsd}
...
\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}Recall the generalized binomial theorem:
\begin{equation}
...
\end{mystadmonition}
\begin{mystadmonition}{mystAmber}{\textbf{Problem 3}}Show that if $h$ satisfies the two-dimensional Laplace equation then $A$ is stationary with
respect to variations that vanish at the boundary.

\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}We need that the Euler-Lagrange equation for (\ref{afsd}) is equivlanet to the tow-dimensional Laplace equation. That is simple: let $f(x,y,h, h_x,h_y)=h_x^2 + h_y^2$,
\begin{equation}
\begin{aligned}
...
\end{mystadmonition}
\begin{mystadmonition}{mystAmber}{\textbf{Problem 4}}Suppose the drumskin has mass $\rho_0$ per unit area, and surface tension $T$. Write down
the Lagrangian controlling the motion of the drumskin and derive the equation of
motion that follows from it.

\end{mystadmonition}
\begin{mystadmonition}{mystGreen}{\emph{Solution}}The kinetic term is given by:
\begin{equation}

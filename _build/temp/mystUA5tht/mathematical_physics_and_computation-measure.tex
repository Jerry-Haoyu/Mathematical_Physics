...
\begin{mystthmdefinition}{Elementary Collection of set}{definition-measure-3}An \textbf{elementary collection} of set is a collection of set $\mathcal E \in P(X)$ such that:
...
\begin{mystthmlemma}{$\mathcal M$ is closed under finite union and $\mu^{*}$ is finitely additive on $\mathcal M$}{lemma-measure-5}Let $\mu^{*}$ be an outer measure:
...
\end{mystthmlemma}
\begin{mystthmproof}{proof of lemma}{proof-measure-6}1.(\textbf{$\mathcal M$ is closed under finite union}) It suffice to show that $A \cup B$ split measure every subset in $X$. That is, $\forall F\subset X$
\begin{equation}
...
\end{mystthmproof}
\begin{mystthmproof}{proof of Carathéodory}{proof-measure-7}We only need to upgrade ``finite'' to ``countable'' in the previous argument.
...
\begin{mystthmdefinition}{Borel Sets}{definition-measure-8}Let $X$ any metric space, the $\sigma$-algebra genereated by the family of open sets in $X$ is called the \textbf{Borel $\sigma$-algebra}, denoted by $\mathcal B_{X}$

\end{mystthmdefinition}
\begin{mystexercise}{h-intervals generate $\mathcal B_{\mathbb{R}}$}{exercise-w3qaihkvjt}\label{exercise-W3QAihkvJt}\label{exercise-w3qaihkvjt}Check $H$ is an elementary set and so it generates an algebra $\mathcal A$. Furthermore, show that the $\sigma$-algebra generated by $\mathcal A$ is   $\mathcal B_{\mathbb{R}}$

\end{mystexercise}
Another terminology:

\begin{mystthmdefinition}{Elementary Decomposition}{definition-measure-9}Suppose $A\in \mathcal A$, since $\mathcal A=\mathcal M(H)$ we can find finite disjoint h-intervals $\{I_j\}_{j=1}^N=\{(a_j,b_j]_j\}_{j=1}^N$ such that:
\begin{equation}
A=\bigcup_{j=1}^N I_j
\end{equation}
...
\begin{mystthmproposition}{}{proposition-measure-10}Let $F:\mathbb{R}\to\mathbb{R}$ to be a right-continuous, increasing function. Define $\mu_0$ as
\begin{equation}
\begin{aligned}
...
\begin{mystthmproof}{$\mu_0$ is a pre-measure}{proof-measure-11}We first need to show that $\mu_0$ is actually well-defined\footnote{Note that in definiting a notion of size $\mu_0$ on $A\in \mathcal A$, we requires $A$ to have a concrete representation $\cup_j I_j$. If the result is independent on
...
\end{mystthmproof}
\begin{mystexercise}{Completeness of $\mathcal M_{\mu^*}$}{exercise-sccwz0830o}\label{exercise-SCCWZ0830O}\label{exercise-sccwz0830o}\end{mystexercise}
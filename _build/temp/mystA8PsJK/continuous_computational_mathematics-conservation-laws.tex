...
\begin{mystadmonition}{mystAmber}{Method of Characterisitcs}\begin{enumerate}
...
\end{mystadmonition}
\begin{mystthmexample}{Constant Wave-speed Advection}{example-conservationlaws-1}Consider the equation $u_t+cu_x=0$ with the initial condition $u(t,0)=\exp( -x^2)$. We parametrize the initial condition as $(t,x,z)=(0,r,\exp( -r^2))$. Fix $r$, the system is:
\begin{equation}
\begin{cases}
   \frac{dt}{ds}&=1 \\
   \frac{dx}{ds}&=c \\
...
\paragraph{The Finite Volume Method}
...
\begin{mystthmdefinition}{Hyperbolic Conservation Law}{definition-conservationlaws-2}There are two subclasses, \textbf{strict conservation law}(with no source term) and \textbf{balance law}(conservation law with source term).
...
\begin{mystthmproposition}{}{proposition-conservationlaws-3}The general homogenous transport equation $u_t+a(x,t)u_x=0$ is a strict conservation law iff $a(x,t)=a(t)$

\end{mystthmproposition}
\subparagraph{The idea behind finite volume}

\begin{mystadmonition}{mystAmber}{Discretization}Finite volume discretize the space into cells $x_i, i\in \mathbb{N}$ being the cell center and $x_{i -1/2},x_{i+1/2}$ being the faces of the cell.

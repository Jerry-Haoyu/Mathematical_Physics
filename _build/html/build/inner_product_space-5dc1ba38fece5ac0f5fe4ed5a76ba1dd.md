...
$$
As desired. 
2. We can direclty compute the derivative:
$$
...
4. Compute the limit
$$
\lim_{\mu \to 0^+}\delta^{(1/2)}_{\mu}=\tilde \delta
$$ 
5. Assuming that for $|x|>\max|\{x_0: \delta^{(1/2)}_{\mu}(x_0)=0\}|$, the convergence of
   $$
       \delta^{(1/2)}_{\mu} \rightarrow \delta^{(1)}
   $$
   is pointwise. Deduce:
$$
\int_{-\infty}^{\infty}\delta^{(1/2)}
$$
:::

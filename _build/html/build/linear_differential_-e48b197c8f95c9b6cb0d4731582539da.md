...
4. We note that the problem $(\Gamma_1, -k)$ is identical to $(\Gamma_2, k)$ after the coordinate transformation $x=-x$($-k$ reverse the direction the in-comingwave is traversing,
we also need to reverse the axis so the direction which the wave travels towards has no reflection. 
5. onsequently, we have two basis in $L$:
$$
\psi &= t_L(-k)\exp(i(-k)(-x)) \\
...
\end{vmatrix} &= ikt_L(-k)t_R(k)-t_L(-k)t_R(k)
$$
:::

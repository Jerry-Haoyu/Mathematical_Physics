...
[^convergent_series_and_tail]: Suppose we are confronted by the series  $s=\sum_{1}^{\infty}a_j$ which converges to $L$.
Need: given $\epsilon>0$, there exists $N(\epsilon)$ such that $\forall N<n<m$ we have $\sum_{n}^m a_j$

We can first pick $M$ such that $\left|\sum_{j=1}^M a_j -L\right| < \epsilon/2$

...
- To show (b) we note that:
  ...
  - Conversely $(a,b)=\cup_{j=1}^{\infty} [a+1/j,b-1/j]$ hence $\mathcal B \subset \mathcal M(\mathcal E_2)$ and hence $\mathcal M(\mathcal T \mathcal(B))=\mathcal M(\mathcal B)\subset \mathcal M(\mathcal E_2)$.(*I'm using (5) again but otherwise we use similar argument to 5 here*)

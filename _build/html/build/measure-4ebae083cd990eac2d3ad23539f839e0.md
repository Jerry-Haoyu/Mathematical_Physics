...
Here is a quick numerical demo for approximating a unit circle with squares on a grid. The resolution $n$ is defined as the number of intervals in $(0,1)$. 
```{figure} media/outer_measure_of_unit_circle.png
```
As we take finer and finer squares, we approach a limit(or the liminf). This number can be used as a notion of size for the set
:::

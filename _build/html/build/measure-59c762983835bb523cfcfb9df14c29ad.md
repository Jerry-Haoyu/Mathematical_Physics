...
2. $(\Longrightarrow)$ Suppose $E$ is $\mu^*$-measurable. Now by (1) we can find a sequence $\{A_n\}_{n=1}^{\infty}$ where $A_n\in \mathcal A_{\sigma}$, each covers $E$ and $\mu^*(A_n)\leq \mu^*(E)+\frac1n$.  By taking the intersection of the sequence and define that as $B$:
$$
B:=\bigcap_{n}A_{n} \in A_{\sigma \delta}
$$
By construction $B$ covers $E$. Since $E\in \mathcal M_{\mu^*}$ where the later is an $\sigma$-algebra and $\mu^*$ is a complete measure on $\mathcal{M}_{\mu^*}$. We can use *continuity-from-above* of outer-meassure if $\mu^*(E)<\infty$ that $\mu^*(A)=\lim_{n \to \infty}\mu^*(A_n)=\mu^*(E)$. 
...
The following exercise is assumed everywhere in mathematics and is not directly related to outer measure. But they are used multiple times, for instance, in subadditivity of outer measure. 
:::{exercise} Order invariance for Series of Positive Terms [^hw_mineyev]
(5*) Prove that any generalized series with summands in $[0, \infty]$ can be summed in any order. Specifically, if $a_j \in [0, \infty]$ for $j \in \mathbb{N} = \{1, 2, 3, \dots\}$, then for any bijection $\mathbb{N} \to \mathbb{N},\ j \mapsto k_j$, the equality $\sum_{j=1}^\infty a_j = \sum_{j=1}^\infty a_{k_j}$ holds. (That is, a nonnegative generalized series can be summed in any order.)(6*) Prove that any two generalized series with summands in $[0, \infty]$ can be summed as one series. Specifically, if $a_j, b_j \in [0, \infty]$ for $j \in \mathbb{N}$, then the equality $\sum_{j=1}^\infty a_j + \sum_{j=1}^\infty b_j = \sum_{j=1}^\infty (a_j + b_j)$ holds.
:::

...
($\Longleftarrow$) Suppose there is such $B$ for $E$(possibly infinite outer-measure) then we can construct $B_n$ and $E_n$ as before. Clearly each $B_n$ still satisfy the property hence we get $E_n \in \mathcal M$ by (2) since each $E_n$ has finite outer-measure. Now we want to show $E \in \mathcal M$ but this is trivial as $\sigma$-algebra is closed under union and$
E= \cup E_n
$
:::

...
$$
Clearly $\mathcal B_x \subseteq \mathcal B'$. Now proceed with the following changes on $\mathcal B'$
```{prf:algorithm}
...
2. While $\cup \mathcal B'' \cap U^c \neq \varnothing $ $x\in U$: 
    1. compute $(c,d)_x=\bigcup_{B\in \mathcal B_x} B$ where $c,d$ is in extended real number system.
    2.  
```
This is justified by the following claim:
- $\boxed{\textbf{Cliam}}$ The union of arbitrary family of open intervals containing $x\in \mathbb{R}$ is a *generalized open interval* 

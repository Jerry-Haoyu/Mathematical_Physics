...
Let $X$ be a complex vector space. An **inner product** is a function from $X \times X \to \mathbb{C}$ 
$$
...
The pair $(X,\langle \cdot, \cdot\rangle)$ is a **pre-Hilbert space**. It is also a Banach Space with norm induced by the inner product:
$$
\|\cdot \| = \langle\cdot, \cdot \rangle^{1/q}
$$
If $X$ is also complete, it is a **Hilbert Space**
:::

## The Riez Representation Theorem 
:::{prf:theorem} Riez Representation Theorem 
Let $f\in H^*$, $\exists !$ $y\in H$ such that $\forall x\in H$ we have:
$$
...
If $\{u_{\alpha}\}_{\alpha \in A}$ is an orthonormal set in $H$, then for any $x\in H$:
$$
...
:::{prf:theorem} Basis Theorem
If $\{u_{\alpha}\}_{\alpha \in A}$ is an orthonormal set then the following are equivalent:
1. (*Completeness*) If $\langle x,u_{\alpha}\rangle =0, \forall \alpha \in A$ then $x=0$
2. (*Parseval's Identity*) $\forall x\in H$ 
$$
\|x\|^2 = \sum_{\alpha \in A} |\langle x, u_{\alpha} \rangle|^2 
$$
1. (*Representation*) $\forall x\in H$ 
$$
...
:::{prf:proof}
$(1)\Longrightarrow (3)$ Let $x\in H$, let $\{\alpha_j\}$ be a enumeartion of nonzero terms in the sum $\sum_{\alpha \in A} \langle x, u_{\alpha} \rangle u_{\alpha}$
$$\sum_{j=1}^{\infty} \langle x, u_{\alpha_j}\rangle u_{\alpha_j}=\sum_{\alpha \in A} \langle x, u_{\alpha}\rangle u_{\alpha} $$
Using orthonormality we have[^generalized_pythagorean]:
$$
...
$$
By Bessel's inequality, we know $\sum_{j}\left| \langle x, u_{\alpha_j} \rangle \right|^2$ converges. This implies[^convergent_series_and_tail] the tail can be made arbitarily small, i.e., $\forall \epsilon >0$, \exists $N(\epsilon)$ such that $\forall N<n<m$ we have: 
$$
...
[^generalized_pythagorean]: This is often referred to as the *Pythagorean's Theorem*, stating that for mutually orthogonal vectors$\{a_j\}_{j=1}^{\infty}$:
$$
...
[^convergent_series_and_tail]: Suppose we are confronted by the series  $s=\sum_{1}^{\infty}a_j$ which converges absolutely to $L$.
Need: given $\epsilon>0$, there exists $N(\epsilon)$ such that $\forall N<n<m$ we have $\sum_{n}^m |a_j| < \epsilon$.
We can pick $N$ such that $\left|\sum_{j=1}^k |a_j| -L < \epsilon/2$, $\forall k\geq N$. But now forall $m>n>N$, we have:
$$
\sum_{n}^m |a_j| &= \sum_{1}^m |a_j| - \sum_1^{n-1} |a_j|   \\
&=\left| \left|\sum_{1}^m |a_j|\right| - \left|\sum_1^{n-1} |a_j| \right|\right| \\
&\leq \left|\sum_{1}^m |a_j|-L\right| - \sum_1^{n-1} |a_j| 
$$

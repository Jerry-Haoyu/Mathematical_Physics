...
**Problems**:
1. Show that in $L,R$, the general solution to the formal operator $H-E$ is:
$$
\psi_k&=A\exp(ikx)+B\exp(-ikx) x\in L 
\psi_k&=C\exp(ikx)+D\exp(-ikx) x\in R
$$
...
\end{cases}
$$
where $\Pi_L$ depicts a left-incident wave with no incoming wave from the right and $\Pi_R$ is the opposite. Show that the solution 
:::

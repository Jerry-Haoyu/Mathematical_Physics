...
where
$$
C= \left\{\sum \mu^*(A_j) | A=\cup A_j\in \mathcal A_{\sigma}, E\subset A \right\}
$$
Hence $\exists c\in C$ such that $c < \mu^*(E)+\epsilon$ by the definition of infimum. Now the map between:
$$
\mathcal A_{\delta} &\to \mathbb{R} \\

...
$$
As desired. 
2. (**Method 1**)First show using the ring homomorphism result:
$$
\delta[f(-g'] &= f(0)\delta[g'] \\
&= -f(0)\delta'[g]
$$
where in the second step we used the definition of distribution derivative:
$$
\delta[-g'] = \delta'[g]
$$
(**Method 2**)We can also show via product rule:
$$
\delta[fg'] &= \delta[(fg)'-f'g] \\
&= \delta[(fg)']-\delta[f'g] \\
&= -\delta'[fg] + \delta'[f'g]
$$
:::

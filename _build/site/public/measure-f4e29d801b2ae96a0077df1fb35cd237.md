...
Suppose $X$ is a set. Let $\mu: A \to \mathbb{R}$ where $A\subset P(X)$
- $$\mu(\varnothing)=0 $$
...
[^measure]: Strictly speaking, we only call such function a measure if it's domain is a $\sigma$-algebra $(X,\mathcal{M})$
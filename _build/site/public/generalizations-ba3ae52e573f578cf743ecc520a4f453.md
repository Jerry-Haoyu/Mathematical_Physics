...
$$
y(z)=\sum_{n=1}^{\infty} a_n\sin(\frac{n\piz}{L})
$$
...
Mgy^{(2)}-YIy^{(4)} &= 0
$$
Denote $\omega_n = \frac{n\pi}{L}$ Substituting into the original ODE to obtain spectral constraint:
$$
-Mg(a_n\omega_n^2)\sin(\omega_n z) &= YI(a_n\omega_n^4)\sin(\omega_n z) \\
-Mg &= YI \omega_n^2
$$
:::
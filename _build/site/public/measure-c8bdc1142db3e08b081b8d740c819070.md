...
m^*(cE + d) = |c|m^*(E)
$$
we claim that every cover  $A=\cup A_j$ for $cE+d$ can be obtained by transforming  a cover for $E$. Specifically, suppose $A=\bigcup (a_j,b_j]$ is a cover for $cE+b$ then 

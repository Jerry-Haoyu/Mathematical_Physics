...
1.
1. Suppose $\mathcal B' \subseteq \mathcal B$ and 
$$
...
4. We only need to show (b) (c) and (e) since (a) and (d) are generalized open intervals and is actualy shown by (5).
- **Case (b): Closed intervals** $\mathcal{E}_2 = \{[a,b]: a < b\}$
  - To show $\mathcal{M}(\mathcal{E}_2) \subseteq \mathcal{B}_\mathbb{R}$:
    - $[a,b] = \cap_{j=1}^{\infty}(a-1/j, b+1/j)$, so each closed interval is a 
      countable intersection of open intervals
    - Hence $\mathcal{E}_2 \subset \mathcal{M}(\mathcal{T}(\mathcal{B})) = \mathcal{B}_\mathbb{R}$
    - Therefore $\mathcal{M}(\mathcal{E}_2) \subset \mathcal{B}_\mathbb{R}$

  - To show $\mathcal{B}_\mathbb{R} \subseteq \mathcal{M}(\mathcal{E}_2)$:
    - $(a,b) = \cup_{j=1}^{\infty}[a+1/j, b-1/j]$, so each open interval is a 
      countable union of closed intervals
    - Hence $\mathcal{B} \subset \mathcal{M}(\mathcal{E}_2)$
    - Therefore $\mathcal{B}_\mathbb{R} = \mathcal{M}(\mathcal{B}) \subset \mathcal{M}(\mathcal{E}_2)$

- **Case (c): Half-open intervals** $\mathcal{E}_3 = \{(a,b]: a < b\}$
- To show $\mathcal{M}(\mathcal{E}_3) \subseteq \mathcal{B}_\mathbb{R}$:
  - $(a,b] = (a,b) \cup \{b\}$ where $(a,b) \in \mathcal{T}(\mathcal{B})$
  - $\{b\} = \cap_{j=1}^{\infty}(b-1/j, b+1/j)$ is a countable intersection of open intervals
  - Hence $(a,b] \in \mathcal{M}(\mathcal{T}(\mathcal{B})) = \mathcal{B}_\mathbb{R}$
  - Therefore $\mathcal{M}(\mathcal{E}_3) \subset \mathcal{B}_\mathbb{R}$

- To show $\mathcal{B}_\mathbb{R} \subseteq \mathcal{M}(\mathcal{E}_3)$:
  - $(a,b) = \cup_{j=1}^{\infty}(a, b-1/j]$, so each open interval is a 
    countable union of half-open intervals
  - Hence $\mathcal{B} \subset \mathcal{M}(\mathcal{E}_3)$
  - Therefore $\mathcal{B}_\mathbb{R} = \mathcal{M}(\mathcal{B}) \subset \mathcal{M}(\mathcal{E}_3)$

**Case (e): Closed rays** $\mathcal{E}_7 = \{[a,\infty): a \in \mathbb{R}\}$
- To show $\mathcal{M}(\mathcal{E}_7) \subseteq \mathcal{B}_\mathbb{R}$:
  - $[a,\infty) = \cap_{j=1}^{\infty}(a-1/j, \infty)$, so each closed ray is a 
    countable intersection of open rays (generalized open intervals)
  - Hence $\mathcal{E}_7 \subset \mathcal{M}(\mathcal{T}(\mathcal{B})) = \mathcal{B}_\mathbb{R}$
  - Therefore $\mathcal{M}(\mathcal{E}_7) \subset \mathcal{B}_\mathbb{R}$

- To show $\mathcal{B}_\mathbb{R} \subseteq \mathcal{M}(\mathcal{E}_7)$:
  - $(a,\infty) = \cup_{j=1}^{\infty}[a+1/j, \infty)$ where each $[a+1/j,\infty) \in \mathcal{E}_7$
  - $(-\infty, b) = \mathbb{R} \setminus [b,\infty)$ where $[b,\infty) \in \mathcal{E}_7$
  - Hence $(a,b) = (a,\infty) \cap (-\infty, b) \in \mathcal{M}(\mathcal{E}_7)$
  - Therefore $\mathcal{B}_\mathbb{R} = \mathcal{M}(\mathcal{B}) \subset \mathcal{M}(\mathcal{E}_7)$

By symmetry, case (c') for $[a,b)$ and case (e') for $(-\infty, a]$ follow similarly.

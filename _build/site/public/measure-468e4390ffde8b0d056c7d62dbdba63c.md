...
$$
Clearly $\mathcal B_x \subseteq \mathcal B'$. Now replace $\mathcal B_x$ by $\bigcup_{B\in \mathcal B_x} B$. This is justified by the following claim:
- $\boxed{\textbf{Cliam}}$ The union of arbitrary family of open intervals containing $x\in \mathbb{R}$ is a *generalized open interval*$$\bigcup_{B\in \mathcal B_x} B\in \{(c,d): c,d\in \mathbb{R}\cup \{\infty, -\infty\}\}$$
:::

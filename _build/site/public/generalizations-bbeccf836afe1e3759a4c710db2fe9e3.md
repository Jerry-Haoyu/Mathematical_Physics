...
$$
y(z)=\sum_{n=1}^{\infty} a_n\sin(\frac{n\pi}{L})
$$
...
Mgy^{(2)}-YLy^{(4)} &= 0
$$
Denote $\omega_n = \frac{n\pi}{L}$Substituting in the fourier series solution:
$$
-\sum_{n=1}^{\infty}Mg(a_n\omega_n^2)\sin(\omega_n z) &= \sum_{n=1}^{\infty}YL(a_n\omega_n^4)\sin(\omega_n z)
$$
:::
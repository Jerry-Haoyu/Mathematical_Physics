...
$$
Note all we need now is $\mu^*(F \cap B^c) \geq  \mu^*(F\cap E^c)$. Also $\mu^*(F\cap E^c) = \mu^*(F\cap (B^c \cup (B\cap E^c))) \leq \mu^*(F\cap B^c)  + \mu^*(F\cap (B\cap E^c))$. Again, by monotonicity and the assumption
$$

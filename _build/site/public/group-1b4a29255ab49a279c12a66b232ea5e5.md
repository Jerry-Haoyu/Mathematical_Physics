...
Likewise, in groups, mating is defiend by a binary operation(binary means takes two inputs) $\cdot : G\times G\to G$. It takes parents $g_1,g_2\in G$ as input and returns the offspring $g_3$. However, there exists an individual that is special known as the *identity*. When mated with any element in the species, the offspring is a copy of its partner. This is an indiviudal 

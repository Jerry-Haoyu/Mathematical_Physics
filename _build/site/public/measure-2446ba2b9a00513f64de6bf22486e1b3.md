...
1. Suppose $\mu_0$ is $\sigma$-finite then $\exists \{X_n\}$, $\mu_0(X_n)<\infty$ such that $X = \cup_n X_n $. Now define $E_n= X_n\cap E$, we can apply (2) to each $E_n$(There eixsts $B_n\in \mathcal A_{\sigma\delta}$ satisfying the property). 
:::

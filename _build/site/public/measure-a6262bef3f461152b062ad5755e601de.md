...
$$
is a cover for $E$, furthermore $cB+d=A$. This can be shown since suppose $c' \in cE+d$ then $\exists e\in E$ such that $c'=ce+d$. Suppose $a_t < ce+d \leq b_t$, then $\frac{a_t-d}{c} < e \leq \frac{b_t-d}{c}$

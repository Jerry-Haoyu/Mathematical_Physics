...
:::{solution} 1dscat
3. Fix a $k$. Consider $\Pi_1$. Form the Wronskian in $L$:
$$
W_L=\begin{vmatrix}
...
4. We note that the problem $(\Gamma_1, -k)$ is identical to $(\Gamma_2, k)$ after the coordinate transformation $x'=-x$. Since $-k$ reverse the direction the in-comingwave is traversing,
we also need to reverse the axis so the direction which the wave travels towards has no reflection. In otherword, we have for $\Gamma_L$
Consequently, we have two basis in $L$:
$$
\psi &= t_L(-k)\exp(i(-k)(-x)) \\
\tilde \psi &= t_R(k)\exp(ikx)
$$
Forming the Wronskian:

$$
\begin{vmatrix}
t_L(-k)\exp(ikx) & t_R(k)\exp(ikx) \\
ikt_L(-k)\exp(ikx) & ikt_R(k)\exp(ikx)
\end{vmatrix} 
= ikt_L(-k)t_R(k)-t_L(-k)t_R(k)
$$
:::

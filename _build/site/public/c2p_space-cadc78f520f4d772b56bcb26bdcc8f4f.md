...
$$
Note that
1. $\lim_{k\to \infty}s_kr^k = 0$ since we know $\lim_{k\to \infty}s_k$ exists and for $|r|<1$, we have $\lim_{r^k}=0$ hence by standard limit product law we have   $$\lim_{k\to \infty}s_kr^k=$
2. $c=(1-r)\frac{c}{1-r}=(1-r)\lim_{k\to\infty}\sum_{j=1}^k cr^j$
:::

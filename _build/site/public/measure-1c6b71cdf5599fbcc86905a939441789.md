...
$(\Longleftarrow)$ Suppose there exists $B\in \mathcal A_{\sigma \delta}$ such that $E\subset B$ and $\mu^*(B\setminus E)=0$. Note any $B\in \mathcal A_{\sigma \delta}$ is $\mu^*$-measurable since $\mathcal A \subset \mathcal M_{}$.

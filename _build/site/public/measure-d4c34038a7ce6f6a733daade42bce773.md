...
- To show (b) we note that:
  - $[a,b]=\cap_{j=1}^{\infty} (a-1/j,a+1/j)$ hence $\mathcal E_2 \subset \mathcal M(\mathcal T(\mathcal B))$ thus $\mathcal M(\mathcal E_2) \subset \mathcal M(\mathcal T(\mathcal B))$ 
  - Conversely $(a,b)=\cup_{j=1}^{\infty} [a+1/j,b-1/j]$

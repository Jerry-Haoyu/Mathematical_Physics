...
Suppose $E\subset \mathbb{R}$, then 

$$
m^*(E)=\inf\left\{\sum_{j=1}^{\infty} (b_j - a_j) {\Huge |} E\subseteq \bigcup (a_j, b_j] \right\}
$$

Consider $E+c := \{e+c | e\in E\}$, then $E\subseteq \bigcup (a_j,b_j] \Longleftrightarrow E+c\subseteq \bigcup_{j=1}^{\infty}(a_j+c, b_j+c]$ hence

$$
m^*(E+c)&=\inf\left\{\sum_{j=1}^{\infty} (b_j+c - (a_j+c)) {\Huge |} E+c\subseteq \bigcup (a_j+c, b_j+c] \right\} \\
&= \inf\left\{\sum_{j=1}^{\infty} (b_j- a_j) {\Huge |} E \subseteq \bigcup (a_j, b_j] \right\} \\
&= m^*(E)
$$

Similarily consider $cE:=\{ce | e\in E\}$, then: 
$$E\subseteq \bigcup (a_j,b_j] \Longleftrightarrow cE \subseteq \bigcup_{j=1}^{\infty}(ca_j, cb_j]$$

Since $e\in E \Rightarrow \exists  e\in (a_l, b_l)$ hence $ce \in (ca_l, cb_l)$ and suppose $h\in cE$ then $\exists e\in E$ such that $h=ce$. Hence $ce \in ()$

...
2. $(\Longrightarrow)$ Suppose $E$ is $\mu^*$-measurable. Now by (1) we can find a sequence $\{A_n\}_{n=1}^{\infty}$ where $A_n\in \mathcal A_{\sigma}$ and $\mu^*(A_n)\leq \mu^*(E)+\frac1n$.  By taking the intersection of the sequence and define that as $B$:
$$

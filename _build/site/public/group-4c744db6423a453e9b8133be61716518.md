...
A group is an algebra structure that is closed. To this aspect, a group is like a biological species. They can breed with each other to get another individual of the same species. The mating of two elemens is described a binary operation $cdot : G\times G\to G$. ,Of course, we would expect the offspring to be different from its parents. However, there exists an individual that is special known as the *identity*. When mated with any element in the species, the offspring 

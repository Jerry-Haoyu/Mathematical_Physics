...
### A Discrete Analog
Let's consider a concrete SL opeartor defined on $[0,1]$ with Dirchlet boundary condition $y(0)=y(1)=0$. Instead of directly discretizing $\mathcal L$, we aim to find a matrix $L$ such that discretizes the bilinear form:
$$
F(y)=\int y\mathcal Ly dx \rightarrow y^TLy
$$
First we note that:
$$
F(y) &= \int p\left(\frac{dy}{dx}\right)^2
$$

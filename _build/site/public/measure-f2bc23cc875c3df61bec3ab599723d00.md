...
Recall that:
$$
m^*([a,b])=\inf\left\{\sum_{j=1}^n b_j - a_j {\Huge |} [a,b]=\bigcup (a_j, b_j] \right\}
$$
For $\leq$ it suffices to find one cover $\{(a_j,b_j]\}$ with $\sum b_j - a_j \leq b-a$. We note $\forall \epsilon > 0$,  $(a-\epsilon/2,b]$ would cover $[a,b]$ and 
$$
\inf\left\{\sum_{j=1}^n b_j - a_j {\Huge |} [a,b]=\bigcup (a_j, b_j] \right\} \leq b - a + \epsilon/2  \leq b-a -\epsilon
$$
where the first inequality follows since $(a-\epsilon/2, b)$ is in the set of $A$ we are taking infimum
:::

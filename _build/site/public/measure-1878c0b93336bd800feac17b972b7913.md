...
$$
For $\leq$ it suffices to find one cover $\{(a_j,b_j]\}$ with $\sum b_j - a_j \leq b-a$. We note $(a-\epsilon,b]$ would cover $[a,b]$ and $b-a\leq b - a + \epsilon \leq \inf\left\{\sum_{j=1}^n b_j - a_j {\Huge |} [a,b]=\bigcup (a_j, b_j] \right\}$
:::

...
$$
We can split $L=L_1+L_2$ such that $\hat y^T L_1 \hat y = \sum_{i=1}^N Np_i(y_{i+1}^2 -2y_{i+1}y_i +y_i^2)$ 
...
\end{pmatrix}
$$
To assemble the matrix for the sum of quadratic forms $\sum_{i=1}^N Np_i(y_{i+1}^2 -2y_{i+1}y_i +y_i^2)$ we can overlap $A_i$ along the diagonal of $L$ with a stride of $1$. Concretely:
```
for i=1,...,N-1
    L[i,i] += Np_i
    L[i+1,i+1] += Np_i

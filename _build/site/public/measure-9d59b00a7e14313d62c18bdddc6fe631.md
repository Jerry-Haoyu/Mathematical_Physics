...
- For $\geq$ we need to show for all cover $A$ as above, 
$$
m^*(A) =\sum_{j=1}^n (b_j - a_j)  \geq b-a 
$$
...
$$
\bigcup_{j=1}^n (a_j,b_j+\frac{\epsilon}{2^j})
$$
without loss of generality, we can assume $b_j \in (a_{j+1},b_{j+1}+\frac{\epsilon}{2^j})$. Now:
$$
m^*(A) &=
$$
:::

...
Recall that:
$$
m^*([a,b])=\inf\left\{\sum_{j=1}^{\infty} b_j - a_j {\Huge |} [a,b]\subseteq \bigcup (a_j, b_j] \right\}
$$
...
$$
\inf\left\{\sum_{j=1}^{\infty} b_j - a_j {\Huge |} [a,b]\subseteq \bigcup (a_j, b_j] \right\} \leq b - a + \epsilon/2  \leq b-a -\epsilon
$$
...
&\leq \epsilon + \sum_{k=1}^n (b_{j_k} -a_{j_k})  \\
&\leq \epsilon + \sum_{k=j}^{\infty} (b_{j} -a_{j})  
$$
:::

:::{exercise}
:label: translation invariance of outer measure
Prove that $m^*$ is translation invariant, and that $m^*(cE) = \vert{}c\vert{}m^*(E)$ for every $E \subseteq \mathbb{R}$ and $c \in \mathbb{R}$
:::


:::{solution} translation invariance of outer measure
:class: dropdown

Suppose $E\subset \mathbb{R}$, we show

$$
m^*(cE + d) = |c|m^*(E)
$$
First we isolate the special case when $c=0$. In that case $cE+d = \{d\}$ and outer measure of singleton is $0$ so the equality is trivial. Now we assume $c\neq 0$. 
we claim that every cover  $A=\cup A_j$ for $cE+d$ can be obtained by transforming  a cover for $E$. Specifically, suppose $A=\bigcup (a_j,b_j]$ is a cover for $cE+d$ then 
$$
B:= 
\begin{cases}
\bigcup_j \left( \frac{a_j-d}{c}, \frac{b_j-d}{c}\right] & c > 0\\ 
\bigcup_j \left( \frac{b_j-d}{c}, \frac{a_j-d}{c}\right] & c < 0
\end{cases}
$$
is a cover for $E$, furthermore $cB+d=A$. We consider the positive case first, the negative case follows similarily. Since for all $e\in E$, $ce+d\in cE+d$ hence $\exists (a_t, b_t]$ such that $ce+d \in (a_t, b_t]$. Hence $e \in \left( \frac{a_t-d}{c}, \frac{b_t-d}{c}\right]$.  Now 
$$
m^*(E) &=\inf \left\{ \sum_j \frac{b_j - a_j}{c} {\Huge{|}} cE+d \subset \bigcup_j \left( \frac{a_j-d}{c}, \frac{b_j-d}{c}\right]   \right\} 
$$
Hence $m^*(E)= m^*(cE+d)/c$ when $c>0$. If $c<0$, then by the same reason, we get:
$$
m^*(E) &=\inf \left\{ \sum_j \frac{a_j - b_j}{c} {\Huge{|}} cE+d \subset \bigcup_j \left( \frac{b_j-d}{c}, \frac{a_j-d}{c}\right]   \right\} 
$$
hence $m^*(E)=-m^*(cE+d)/c$. As desired.
:::

:::{exercise} Topological size and Measure-theoretic size can be Juxtaposed 
:label: Topological size and Measure-theoretic size can be Juxtaposed 
Construct, for every $\varepsilon > 0$, an open dense subset $U \subseteq \mathbb{R}$ with $m(U) < \varepsilon$. Conclude that its complement is a closed, nowhere dense set of infinite measure: topological size and measure-theoretic size are very different things.
:::

:::{solution} Topological size and Measure-theoretic size can be Juxtaposed 
Consider $\mathbb{Q}\subset \mathbb{R}$ which is dense. Since $\mathbb{Q}$ is denumerable, consider an enumeration $Q = q_1,q_2,...$. Consider the following construction(psuedo-algorithm): 
```{prf:algorithm}
**Input:** $\epsilon > 0$
1. $A_{\epsilon}\gets \varnothing$
2. For $j=1,2...$ 
    1. $A_j\gets B^o_{\epsilon/2^{j+1}}(q_j)$ 
    2. $A_{\epsilon}= A\cup A_j$
```
where $B^o_{r}(x)$ denotes the open ball of radius $r$ around $x$ in $\mathbb{R}^d$. Now clearly 
$$
m(A_{\epsilon})\leq \sum_{j=1}^{\infty}\frac{\epsilon}{2^j } = \epsilon
$$
The set is open because it is a countable union of open balls. Threrefore its complement is closed and has measure $\infty -\epsilon=\infty$(by additivity of measure). It's left to show that the complement is nowhere dense. We need $(\overline{\mathbb{R}\setminus A_{\epsilon}})^{\circ}=\varnothing$, since $\mathbb{R}\setminus A_{\epsilon}$ is closed it suffices to show $\mathbb{R}\setminus A_{\epsilon}$ has empty interior, i.e., contains no open interval. Suppose $(a,b)\subset \mathbb{R}\setminus A_{\epsilon}$, then $\forall j,  B_{\epsilon/2^{j+1}}^o(q_j) \cap (a,b) = \varnothing$. But rationals are dense in $\mathbb{R}$ hence there must exists $q\in (a,b), q\in \mathbb{Q}$, contradicts. 
:::

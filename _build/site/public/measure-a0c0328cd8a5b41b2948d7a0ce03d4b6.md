...
$$
\epsilon + \sum_{j=1}^n (b_j - a_j)  \geq b-a 
$$
Let $A=\cup A_j, A_j=\{(a_j,b_j]\}_{j=1}^{\infty}$ be an arbitary cover. Consider $A'_j=(a_j,b_j+\frac{\epsilon}{2^j})$,  clearly $[a,b]\subset \cup A'_j $. By compactness, we can pick a finite subcover:
$$
\bigcup_{k=1}^n (a_{j_k},b_{j_k}+\frac{\epsilon}{2^{j_k}})
$$
with out loss of generality, we can assume $b_{j_k} \in (a_{{j_k}+1},b_{{j_k}+1}+\frac{\epsilon}{2^{j_k}})$. Now:
$$
\sum_{j=1}^n (b_j + \frac{\epsilon}{2^j} -  a_j)
$$
:::

...
$$
By construction $B$ covers $E$. Since $E\in \mathcal M_{\mu^*}$ where the later is an $\sigma$-algebra$ and $\mu^*$ is a complete measure on $\mathcal M_{\mu^*}$. We can use right continuity of outer-meassure if $\mu^*(E)<\infty$ that $\mu^*(A)=\lim_{n \to \infty}\mu^*(A_n)=\mu^*(E)$. 

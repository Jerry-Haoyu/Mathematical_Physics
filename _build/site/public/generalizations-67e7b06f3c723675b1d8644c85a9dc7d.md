...
&= \sum_{n=1}^{\infty}\underbrace{a_n(Mg\omega_n^2-YI\omega_n^4)}_{b_n}\sin(\omega_n z)
$$

Hence a trivial solution is $a_n=0, \forall n\geq 1$ which corresponds to a *stable rod*. 

The rod buckles if, for a *nontrivial solution* $(a_n\neq 0)$, the energy functional is at a local minimum for the 
first mode. In otherword, we have a positive eigenvalue in the first dimension.

We need to examine the hessian in this case.
...
$$
\widehat H = \mathrm{diag}(\{(Mg\omega_n^2-YI\omega_n^4)\}_{n=1}^{\infty})
$$

Local minimum requires positive-definiteness, which is :
$$
Mg\omega_n^2-YI\omega_n^4 &> 0  \\

...
[^normalization]: Since iterative eigensolvers gives eigenvectors of arbitrary magnitude $v_j=a\sin(j\pi(x))$, we need to normalize properly by adjusting $\pm \sin(j\pi x)$ to $\sqrt{2}\sin(j
\pi(x))$
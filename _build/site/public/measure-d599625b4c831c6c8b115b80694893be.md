...
($\Longleftarrow$) Suppose there is such $B$ for $E$(possibly infinite outer-measure) then we can construct $B_n$ and $E_n$ as before. Clearly each $B_n$ still satisfy the property hence we get $E_n \in \mathcal M$ $\mu^*$-measurable by (2) since each $E_n$ has finite outer-measure. Now since $E\in \mathcal M$
:::

...
$$s_k :=\sum_{j=1}^k a_j $$
 ...
 To do so, we would like to utillize the fact the series is convergent. One way to do so is  need to manipulate the quantity such that $s_j-c$
$$
...
$$
\left| (1-r)\sum_{j=1}^{N} (s_j-c)r^j + (1-r)\sum_{j=N+1}^{\infty} (s_j-c)r^j\right|
$$
:::

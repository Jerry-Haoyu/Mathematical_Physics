...
$$
so we can easily find a delta ball of radius $\frac{\epsilon}{k}$ given an epsilon ball such that the delta ball is mapped into the epsilon ball. This is actually the case for any linear operator on finite-dimensional space. Looking at how we obtained continuity from 

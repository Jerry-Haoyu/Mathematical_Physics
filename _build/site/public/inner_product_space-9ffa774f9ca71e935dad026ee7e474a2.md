...
$$
As desired. 
2. First show using the ring homomorphism result:
$$
(f\delta, \partial \psi) &= (f(0)\delta, \psi) \\
                &= -(\partial f(0)\delta,\psi) \\
                &= -(f(0)\delta'(x),\psi)
$$
We can also show via product rule:
$$
(f\delta, \partial \psi) &= -(\partial(f\delta),\psi) \\
&= -(\partial f\delta + f\partial \delta \)
$$
:::

...
With this key proposition in hand, we can already algorithmically construct a triangular matrix representation for $T\in \mathcal L(V)$ where $V$ is over algebraically closed $\mathbb{F}$.The idea being we can recursively (1) find an eigenvalue (2) factor out th

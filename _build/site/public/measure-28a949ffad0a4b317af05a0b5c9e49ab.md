...
The question would be, when can we find such a function. Untrivially, this is not possible when $A\subset P(X)$, in otherword, it is not possible to define a sense of volume on arbitrary set. What if $X=\mathbb{R}$, does 

...
We first need to show that $\mu_0$ is actually well-defined[^well-defined]. 
:::

[^well-defined]: Note that in definiting a notion of size on $A\in \mathcal A$, we requires $A$ to have a concrete representation $\cup_j I_j$. If the result is independent on 
how we choose this representation(i.e. different elementary decomposition$\{I_j\}$), then the notion of size 
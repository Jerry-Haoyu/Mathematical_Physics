...
\end{cases}
$$
is a cover for $E$, furthermore $cB+d=A$. We only consider the positive case, the negative case follows. Since for all $e\in E$, $ce+d\in cE+d$ hence $\exists (a_t, b_t]$ such that $ce+d \in (a_t, b_t]$. Hence $e \in \left( \frac{a_t-d}{c}, \frac{b_t-d}{c}\right]$.  Now 
$$
m^*(E) &=\inf \left\{ \sum_j \frac{b_j - a_j}{c} {\Huge{|}} cE+d \subset \bigcup_j \left( \frac{a_j-d}{c}, \frac{b_j-d}{c}\right]   \right\} 
$$
Hence $m^*(E)= m^*(cE+d)/c$ when $c>0$. If $c<0$, then by the same 
:::

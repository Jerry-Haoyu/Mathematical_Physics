...
If we consider linear functions $f:\mathbb{R}\to \mathbb{R}$, it seems continuity is free since we can always write $f(x)=kx$ hence:
$$
\|f(x)-f(x_0)\|=\|k(x-x_0)\|=k\|x-x_0\|
$$
so we can easily find a delta ball of radius $\frac{\epsilon}{k}$ given an epsilon ball such that the delta ball is mapped into the epsilon ball. This is actually the case for any linear operator on finite-dimensional space. For more complicated linear operators $T:\mathbb{R}^n \to \mathbb{R}^n$ ,for instance, we can first show that the the 

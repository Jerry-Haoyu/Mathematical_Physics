...
$(\Longleftarrow)$ Suppose there exists $B\in \mathcal A_{\sigma \delta}$ such that $E\subset B$ and $\mu^*(B\setminus E)=0$. Note any $B\in \mathcal A_{\sigma \delta}$ is $\mu^*$-measurable.

We would like
$$
\mu^*(E) \overbrace{}= \mu^*(E\cap B) + \cancel{\mu^*(E \cap B^c)}
$$
The second term is 0 since $E\subset B$ hence $E\cap B^c =\varnothing$.

$$
\mu^*(B) &= \mu^*(B\cap E) + \cancel{\mu^*(B\cap E^c)}
$$
where the second term is 0 by assumption.
But now:
:::

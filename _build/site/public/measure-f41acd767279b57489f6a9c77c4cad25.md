...
:::{exercise} Topological size and Measure-theoretic size can be Juxtaposed [^hw_mineyev]
...
```
where $B^o_{r}(x)$ denotes the open ball of radius $r$ around $x$ in $\mathbb{R}^d$. Now clearly $A_{\epsilon}$ is dense since $\mathbb{Q}\subset A_{\epsilon}$ and 
$$

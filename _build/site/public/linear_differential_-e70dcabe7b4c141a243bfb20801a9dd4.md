...
\end{cases}
$$
where $\Pi_L$ depicts a left-incident wave with no incoming wave from the right and $\Pi_R$ is the opposite. Show that the solution(in $L,R$) for the BC $\Pi_1, k>0$ is:
$$
...
4.Fix $k$. We repeat the same procedure with $\{\psi_k^L,\psi_k^R\}$
:::

...
This analogy is good as long as we  only consider the closedness aspect. A group is fundamentally different(well, firstly the binary operation is deterministic while breeding is probablistic and they only produce one output). Also, group posses more strucutre known :

...
T_{n-1}:=T_n -I\lambda 
$$
- **Claim**: $\mathrm{range}(T_{n-1})$ is a subspace of dimension less than $n$. 
    - Indeed since by definition, $T_{n-1}$ is not injective hence not surjective, so $\dim(\mathrm{range}(T_{n-1})) < n$.
As a result of the claim and the inductive hypothesis, $T_{n-1}$ is *triangularizable*. 

Let $\beta = \{v_1,...,v_{n-1}\}$ be such a basis so that $[T_{n-1}]_{\beta}$ is upper triangular. 
Now:
$$
Tu=(T-I\lambda)u + \lambda u =(T_{n-1} + \lambda I)u
$$
Since $T_{n-1}$ is triangularizable, $\forall j \leq n-1$, $T_{n-1}v_j\in \langle v_1,...,v_j \rangle$. Now consider $\beta'=\beta \cup \{v\}$,  $\lambda v_j $ clealry also lands in $\langle v_1,...,v_j \rangle$. As a result, 

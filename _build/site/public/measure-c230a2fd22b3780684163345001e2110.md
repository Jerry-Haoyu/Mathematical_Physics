...
Now what's left to show is (1) Only countable number of $\mathcal B_x'$ is needed. (2) each $\mathcal B_x'$ is countable. The later is obvious by construction. The former can be argued by countability of $\mathbb{Q}$. Pick a $q\in \mathbb{Q}$ in one of the open interval of $\mathcal B_x'$, this is always possible by density of rationals. Suppose uncountable $\mathcal B_x'$
:::

...
$$
so we can easily find a delta ball of radius $\frac{\epsilon}{k}$ given an epsilon ball such that the delta ball is mapped into the epsilon ball. This is actually the case for any linear operator on finite-dimensional space. For more complicated linear operators $f:\mathbb{R}^n \to \mathbb{R}^n$ ,for instance, we can first show that the 

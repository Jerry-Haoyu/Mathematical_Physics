...
$$
Also $\mu^*(F\cap E^c) = \mu^*(F\cap (B^c \cup (B\cap E^c))) \leq \mu^*(F\cap B^c)  + \mu^*(F\cap (B\cap E^c))$. Again, by monotonicity and the assumption
$$
\mu^*(F\cap (B\cap E^c))\leq \mu^*(B\cap E^c) = 0
$$
Therefore $$ \mu^*(F)\geq  \mu^*(F\cap E) + \mu^*(F\cap E^c)
$$
:::

...
- $\boxed{\textbf{Cliam 1}}$ The union of arbitrary family of open intervals containing $x\in \mathbb{R}$ is a *generalized open interval* 
  ...
  - I claim the union is just $(c,d)$ where $c=\inf \{a:(a,b)\in \mathcal B_x\}$ and $d=\sup \{b:(a,b)\in \mathcal B_x\}$ where we allow $\infty$ and $-\infty$. Clearly $(c,d)$ covers $\cup \mathcal B_x$. It suffice to show $\mathcal B_x $ covers $(c,d)$. Suppose $\exists y \in \cup \mathcal B_x$ and $y\notin (c,d)$. There must exists $(a_y,b_y)\in \mathcal B_x$
:::

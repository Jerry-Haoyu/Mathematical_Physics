...
:::{solution} folland_19
:class: dropdown
$(\Longrightarrow)$ Suppose $E$ is $\mu^*$-measurable. Then so is $E^c$ (as $\mathcal M_{\mu^*}$ is a $\sigma$-algebra), and since $X = E \sqcup E^c$, finite additivity of $\mu^*|_{\mathcal M_{\mu^*}}$ gives
$$
\mu^*(X) = \mu^*(E) + \mu^*(E^c)
$$
Since $X\in \mathcal A$, the extension theorem gives $\mu^*(X)=\mu_0(X)$, so
$$
\mu^*(E) = \mu_0(X) - \mu^*(E^c) = \mu_*(E)
$$

$(\Longleftarrow)$ Suppose $\mu^*(E)=\mu_*(E)$, i.e.
$$
\mu^*(E) + \mu^*(E^c) = \mu_0(X) = \mu^*(X)
$$
Note $\mu_0(X)<\infty$ (finite premeasure), so $\mu^*(F)\leq \mu^*(X)<\infty$ for every $F\subset X$ — this is exactly what lets us freely subtract finite quantities below.

Fix $\epsilon>0$. By [](folland_problem_18) part (1) applied to $E$ and to $E^c$ separately, we can find $A,A'\in \mathcal A_\sigma\subset \mathcal M_{\mu^*}$ with
$$
E\subset A,\quad \mu^*(A)\leq \mu^*(E)+\epsilon, \qquad E^c\subset A', \quad \mu^*(A')\leq \mu^*(E^c)+\epsilon
$$
Since $E\subset A$ and $E^c \subset A'$, we have $A\cup A' = X$. As $A,A'\in \mathcal M_{\mu^*}$, finite additivity (inclusion–exclusion for measures) gives
$$
\mu^*(X) + \mu^*(A\cap A') = \mu^*(A\cup A') + \mu^*(A\cap A') = \mu^*(A)+\mu^*(A') \leq \mu^*(E)+\mu^*(E^c)+2\epsilon = \mu^*(X)+2\epsilon
$$
Since $\mu^*(X)<\infty$ we may subtract it from both sides:
$$
\mu^*(A\cap A') \leq 2\epsilon
$$
Now $A\setminus E = A\cap E^c \subset A\cap A'$ (using $E^c\subset A'$), so by monotonicity $\mu^*(A\setminus E)\leq 2\epsilon$.

Applying this with $\epsilon = \frac1n$ produces $A_n\in\mathcal M_{\mu^*}$, $E\subset A_n$, $\mu^*(A_n\setminus E)\leq \frac2n$. Let $B:=\bigcap_n A_n \in \mathcal M_{\mu^*}$; then $E\subset B$ and $B\setminus E \subset A_n\setminus E$ for every $n$, so
$$
\mu^*(B\setminus E)\leq \mu^*(A_n\setminus E) \leq \frac2n \xrightarrow{n\to\infty} 0
$$
hence $\mu^*(B\setminus E)=0$.

We now finish exactly as in the $(\Longleftarrow)$ direction of [](folland_problem_18) part (2): for any $F\subset X$, since $B$ is measurable and $E\subset B$,
$$
\mu^*(F) = \mu^*(F\cap B)+\mu^*(F\cap B^c) \geq \mu^*(F\cap E) + \mu^*(F\cap B^c)
$$
and
$$
\mu^*(F\cap E^c) = \mu^*\big(F\cap(B^c\cup(B\cap E^c))\big) \leq \mu^*(F\cap B^c) + \underbrace{\mu^*(F\cap(B\cap E^c))}_{\leq\, \mu^*(B\setminus E)\,=\,0}
$$
so $\mu^*(F\cap B^c)\geq \mu^*(F\cap E^c)$, giving $\mu^*(F)\geq \mu^*(F\cap E)+\mu^*(F\cap E^c)$. Combined with subadditivity (the reverse inequality, always true), $E$ is $\mu^*$-measurable. As desired.
:::

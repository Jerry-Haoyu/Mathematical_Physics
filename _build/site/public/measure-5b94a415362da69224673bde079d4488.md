...
$$
The set is open because it is a countable union of open balls. Threrefore its complement is closed and has measure $\infty -\epsilon=\infty$(by additivity of measure). It's left to show that the complement is nowhere dense. We need $(\overline{A_{\epsilon}})^{\circ}=\varnothing$, it suffices to show $A_{\epsilon}$ contains no open interval since $A_{\epsilon}$.
:::

...
$$
Hence $\exists c\in C$ such that $c < \mu^*(E)+\epsilon$ by the definition of infimum. Now the map between open covers and their area is clearly bijective by construction:
$$
\{A\in \mathcal A_{\delta} : E\subset A\} &\to \mathbb{R} \\
A &\mapsto \sum_j \mu_0(A_j)
$$
hence we can find $A\in A_{\delta}$ that correponds to $c$.
:::

...
:::{attention} Exercise
1. Remove the masking code snippet(implementing Dirchlet boundary condition) in the naive SL class `SturmLiouville`. What is the resulting eigenvectors? Explain the result.
2. Try a different discretization with center difference or back difference. 
   - Is the resulting matrix $L$ still symmatetric ? 
   - Center differencing scheme results in a numerical artifcat, what is the numerical artifact? Try explaining why such artifact occur(*Hint: Note that even and oddd indicies are dec*)
:::

...
:::{prf:theorem} $C_{2\pi}$  Space is Spanned by Fourier Basis
...
So, in otherword, in order to shown we can do fourier expansion(that is, regarding $C_{2\pi}$ as an inner product space endorsed with $C_$), we would have to show its convergence for arbitrary $f\in C_{2\pi}$.
...
$$s_k :=\sum_{j=1}^k a_j $$
 ...
 The goal is to use $\delta$ to control the following quanitity $$E:=\left|\lim_{k\to \infty}s_j - c\right| $$
...
$$
1. For the tail sum, we notice that, by the convergence of the sequence $s_j$, we can pick an $N$ such that $\forall j > N$, $|s_j-c|<\epsilon/2$. Hence:
$$
(1-r)\sum_{j=N+1}^{\infty} |s_j-c|r^j &\leq \frac{\epsilon}{2}(1-r)\sum_{j=N+1}^{\infty} r^j \\ 
&= \frac{\epsilon}{2}
$$
1. For the leading sum, we would like to control:
$$
